...
def _color_on() -> bool:
    """Colorize only when stderr is a tty and NO_COLOR is unset (no-color.org)."""
    return not os.environ.get("NO_COLOR") and sys.stderr.isatty()


def log(msg: str, level: str = "info", tag: str = _TAG) -> None:
    """One log line to stderr: dim component tag, level-colored severity, message.
    level="header" renders the message bold with no severity word (phase banner).
    Matches install-pkg.sh's palette so interleaved output reads as one system."""
    if _color_on():
        dim, reset, bold = "\033[2m", "\033[0m", "\033[1m"
        if level == "header":
            print(f"{dim}{tag}{reset} {bold}{msg}{reset}", file=sys.stderr)
        else:
            color = _LEVEL_COLORS.get(level, "")
            print(f"{dim}{tag}{reset} {color}{level:<5}{reset} {msg}", file=sys.stderr)

    elif level == "header":
        print(f"{tag} {msg}", file=sys.stderr)

    else:
        print(f"{tag} {level:<5} {msg}", file=sys.stderr)
...
def seed_config(template: Path) -> None:
    ...
    log(f"seeded config -> {CONFIG_PATH}")
...
def expand_ref(config: dict, ref: str) -> list[tuple[str, str]]:
    ...
    if step == "*":
        pairs = [
            (s, module) for s in STEP_ORDER if module in modules_in_step(config, s)
        ]
        if not pairs:
...
def install_packages(repo_home: Path, names: list[str], dry_run: bool) -> None:
    ...
    if not names:
        log("packages: none to install", level="header")
        return
    ...
    if dry_run:
        log(
            f"packages: would install {len(names)} via install-pkg.sh -f:",
            level="header",
        )
        for name in names:
    ...
    try:
        with os.fdopen(fd, "w") as fh:
            fh.write("\n".join(names) + "\n")
        log(f"packages: installing {len(names)} via {installer.name}", level="header")
        try:
...
def register_xdg_user_dir(label: str, path: Path, dry_run: bool) -> None:
    ...
    if shutil.which("xdg-user-dirs-update") and _register_xdg_via_tool(
        label, path, dry_run
    ):
        return
...
def _register_xdg_via_tool(label: str, path: Path, dry_run: bool) -> bool:
    ...
    try:
        ...
        log(
            f"xdg: xdg-user-dirs-update failed for {xtype} "
            f"(exit {exc.returncode}); falling back to user-dirs.dirs",
            level="warn",
        )
        return False

    log(f"xdg: {xtype} -> {path} (via xdg-user-dirs-update)")
    return True
...
def _register_xdg_via_file(label: str, path: Path, dry_run: bool) -> None:
    ...
    if dry_run:
        log(f"xdg: would set {entry} in {dirs_file}")
        return
    ...
    log(f"xdg: {key} -> {_xdg_value(path)} (via user-dirs.dirs)")
...
def _apply_one_directory(module: str, item: object, dry_run: bool) -> None:
    ...
    if dry_run:
        log(f"mkdir -p {path}" + (f" (mode {mode})" if mode else ""))
    else:
        path.mkdir(parents=True, exist_ok=True)
        if mode is not None:
            path.chmod(bits)
        log(f"dir {path}" + (f" (mode {mode})" if mode else ""))
...
def apply_directories(
    config: dict, modules: list[tuple[str, str]], dry_run: bool
) -> None:
    ...
    log(
        f"directories: {total} item(s) across {len(dir_modules)} module(s)",
        level="header",
    )
...
def apply_git(config: dict, modules: list[tuple[str, str]], dry_run: bool) -> None:
    ...
    log(f"git: {total} repo(s) across {len(git_modules)} module(s)", level="header")
...
def _parse_file_item(
    ...
    if content is not None and source is not None:
        sys.exit(
            f"files.{module}: item has both content and source (choose one): {item!r}"
        )
...
def _ensure_file(module: str, path: Path) -> None:
    ...
    try:
        ...
        sys.exit(
            f"files.{module}: cannot create {path}: {exc} "
...
def _write_file(module: str, path: Path, content: str) -> None:
    try:
        ...
        sys.exit(
            f"files.{module}: cannot write {path}: {exc} "
            "(does its parent dir exist? add a directories module to requires)"
        )
...
def _copy_file(module: str, src: Path, path: Path) -> None:
    if not src.exists():
        sys.exit(
            f"files.{module}: source not found: {src} "
...
def apply_files(config: dict, modules: list[tuple[str, str]], dry_run: bool) -> None:
    ...
    log(f"files: {total} item(s) across {len(file_modules)} module(s)", level="header")
...
def _fetch_with_progress(url: str, out, label: str) -> None:
    ...
    dim, reset = ("\033[2m", "\033[0m") if _color_on() else ("", "")
        ...
        while chunk := resp.read(1 << 16):
            ...
            if cur > step:
                ...
                msg = (
                    ...
                    f"({done * 100 // total}%)"
                    if total
                    else f"{label}: {done / 2**20:.1f} MiB"
                )
                if tty:
                    print(
                        f"\r{dim}{_TAG}{reset} {msg}",
                        end="",
                        file=sys.stderr,
                        flush=True,
                    )
...
def _install_font(module: str, name: str, dry_run: bool) -> bool:
    ...
    if dry_run:
        log(f"fetch {url} -> {dest}/")
        return True

    log(f"fetching {name} ({NERD_FONTS_VERSION})")
...
def _rebuild_font_cache(dry_run: bool) -> None:
    ...
    if dry_run:
        log("would run fc-cache -f")
        return

    if not shutil.which("fc-cache"):
        log(
            "fc-cache not found; skipping cache rebuild "
    ...
    try:
        ...
        log("font cache rebuilt (fc-cache -f)")

    except subprocess.CalledProcessError as exc:
        log(
            f"fc-cache failed (exit {exc.returncode}); "
...
def apply_fonts(config: dict, modules: list[tuple[str, str]], dry_run: bool) -> None:
    ...
    log(f"fonts: {total} font(s) across {len(font_modules)} module(s)", level="header")
...
def _repo_tool_dirs(
    config: dict, modules: list[tuple[str, str]], repo_home: Path
) -> list[str]:
...
def widen_sparse_checkout(
    config: dict, modules: list[tuple[str, str]], repo_home: Path, dry_run: bool
) -> None:
    ...
    log(f"sparse-checkout: widen for {', '.join(dirs)}", level="header")
...
def _backup_real(module: str, dest: Path) -> None:
    ...
    if backup.exists():
        sys.exit(
            f"links.{module}: {dest} is a real file and {backup} already exists; "
            "resolve manually"
        )
    dest.rename(backup)
...
def _apply_one_link(module: str, item: object, dry_run: bool) -> None:
    ...
    if copy:
        ...
        if not src.exists():
            sys.exit(
                f"links.{module}: source not found: {src} "
        ...
        try:
            ...
            sys.exit(
                f"links.{module}: cannot copy {src} -> {dest}: {exc} "
    ...
    if dest.is_symlink() and dest.readlink() == src:
        log(f"present: {dest} -> {src} (skip)")
        return

    needs_backup = dest.exists() and not dest.is_symlink()
    if dry_run:
        if needs_backup:
            log(f"  backup {dest} -> {dest.name}.bak")
        log(f"  link {dest} -> {src}")
        return
    ...
    try:
        ...
        sys.exit(
            f"links.{module}: cannot link {dest} -> {src}: {exc} "
            "(parent dir missing? add a directories module to requires)"
        )
...
def apply_links(config: dict, modules: list[tuple[str, str]], dry_run: bool) -> None:
    ...
    log(f"links: {total} item(s) across {len(link_modules)} module(s)", level="header")
...
def _parse_service_item(
    ...
    if scope not in ("system", "user"):
        sys.exit(
            f"services.{module}: invalid scope {scope!r} (want system|user): {item!r}"
        )
...
def _run_service_action(
    module: str, name: str, action: str, scope: str, dry_run: bool
) -> None:
    ...
    if scope == "system" and os.geteuid() != 0:
        if not dry_run and not shutil.which("sudo"):
            sys.exit(
                f"services.{module}: need root or sudo for system service "
    ...
    try:
        ...
        sys.exit(
            f"services.{module}: '{action} {name}' failed "
            f"(systemctl exited {exc.returncode})"
        )

    log(f"  {action} {name} ({scope})")
...
def _apply_one_service(module: str, item: object, dry_run: bool) -> None:
    ...
    if enabled is None and started is None:
        log(f"  service {name}: nothing to do (no enabled/started)")
        return

    if enabled is not None:
        _run_service_action(
            module, name, "enable" if enabled else "disable", scope, dry_run
        )

    if started is not None:
        _run_service_action(
            module, name, "start" if started else "stop", scope, dry_run
        )
...
def apply_services(config: dict, modules: list[tuple[str, str]], dry_run: bool) -> None:
    ...
    log(
        f"services: {total} item(s) across {len(svc_modules)} module(s)", level="header"
    )

    if not shutil.which("systemctl"):
        if not dry_run:
            sys.exit(
                "services: systemctl not found (this step requires systemd); "
                "run on a systemd host or drop the services module(s) from the profile"
            )
        log("note: systemctl not found; a real run would fail here", level="warn")
...
def _shell_argv(module: str, command: str, sudo: bool, dry_run: bool) -> list[str]:
    ...
    if sudo and os.geteuid() != 0:
        if not dry_run and not shutil.which("sudo"):
            sys.exit(
                f"commands.{module}: 'sudo = true' but not root and no sudo "
...
def _apply_one_command(module: str, item: object, dry_run: bool) -> None:
    ...
    if dry_run:
        if creates is not None and expand_path(creates).exists():
            log(f"  would skip: {label} (creates exists)")
            return
        ...
        log(f"  would run: {' '.join(argv)}{suffix}")
        return
...
def apply_commands(config: dict, modules: list[tuple[str, str]], dry_run: bool) -> None:
    ...
    log(
        f"commands: {total} item(s) across {len(cmd_modules)} module(s)", level="header"
    )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "profile", help="profile name defined in config.toml [profiles]"
    )
    parser.add_argument(
        "-n",
        "--dry-run",
...
def main(argv: list[str]) -> int:
    ...
    log(f"profile {args.profile!r}: resolved {len(modules)} module(s)", level="header")
    ...
    if later:
        log(f"{len(later)} later module(s) not yet applied: {', '.join(later)}")
